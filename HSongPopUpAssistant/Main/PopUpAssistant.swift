...
public class PopUpAssistant: NSObject {
    ...
    /// 自定义通知
    public func showNoticeCustomView(_ customView: PopUpBaseView, position: PopUpAlertPosition = .center, useCover: Bool = true, showTimeSecond: Double = 3) {
    ...
    public func showCountdownView(withTitle title: String, andTipContent tipContent: String, andAutoBtnName autoBtnName: String, andTime time: Int, enterAction: @escaping (_ isAuto: Bool) -> Void, closeAction: @escaping () -> Void) {
    ...
    ///   - leftOperation: 左侧操作
    ///   - rightOperation: 右侧操作
    public func showTwoBtnView(title: String = "提示", content: String, leftBtnName: String = "取消", rightBtnName: String = "确定", leftOperation: @escaping (_ btnType: String) -> Void, rightOperation: @escaping () -> Void) {
    ...
    ///   - rightOperation: 右侧操作
    ///   - agreements: 协议
    public func showTwoBtnAndAgreementView(
        title: String = "提示",
        content: String,
...
public struct PopUpAgreementModel {
