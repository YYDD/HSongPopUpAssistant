//
//  PopUpNoticeCustomView.swift
//  MyViewFactory
//
//  Created by Wanglei on 2024/3/25.
//

import UIKit

class PopUpNoticeCustomView: UIView, PopUpAssistantDelegate {
    ...
    //MARK: - 逻辑区
    /// 提示弹窗
    func alert(customView: PopUpBaseView, position: PopUpAlertPosition, useCover: Bool = true, showTimeSecond: Double = 1.5) {
        ...
        switch position {
        case .center:
            self.popFromCenter(customView: customView, showTimeSecond: showTimeSecond)
        case .bottom:
            self.popFromBottom(customView: customView, showTimeSecond: showTimeSecond)
        case .top:
            self.popFromTop(customView: customView, showTimeSecond: showTimeSecond)
        }
    ...
    func popFromBottom(customView: PopUpBaseView, showTimeSecond: Double) {
    ...
    func popFromCenter(customView: PopUpBaseView, showTimeSecond: Double) {
        customView.hiddenAndShowBlock = { isHidden in
            self.hiddenAndShow(isShow: !isHidden)
        }
    ...
    func popFromTop(customView: PopUpBaseView, showTimeSecond: Double) {
        customView.hiddenAndShowBlock = { isHidden in
            self.hiddenAndShow(isShow: !isHidden)
        }
        // 检查是否应使用 frame-based 动画
